$(function(){
    let get_current_user = () => {
        ...
        return $.get(`/ap/v1/users/${$current_user_id}/`)
    ...
    $.get(`/ap/v1/stories/${$story_id}`, function (response, status) {
        if (status == 'success') {
            let content = ''
    ...
    /*$.get(`/ap/v1/users/${$current_user_id}/following_stories/`, function ($response, $status) {
        if ($status == 'success') {
    ...
    function fetchStories(url, status) {
        if (loading) return
        loading = true
        let $url = `/ap/v1/topics/${$current_user_id}/foryou_stories?page=${page}&per_page=${perPage}`
        if (url) {
            $url = url
        }
    ...
    $('.topic-btn').on('click', function () {
        ...
        loading = false;
        fetchStories(`/ap/v1/topics/${topic_id}/${$current_user_id}/stories?page=${page}&per_page=${perPage}`, 'topic')
    })
    ...
    $('.following').on('click', function () {
        ...
        $(this).addClass('rounded-sm text-white bg-mediumpurple')
        fetchStories(`/ap/v1/users/${$current_user_id}/following_stories?page=${page}&per_page=${perPage}`)
    })
